...
function tankvol(h, d, vt) {
  const radius = d/2;
  const volume = Math.PI * radius * radius;
  console.log(volume);
  const heightOfLiq = radius - h;
  let theta = Math.acos(((radius) - h)/(radius));
  let remainingVol = ((2*Math.PI - 2*theta)/ (2*Math.PI));  
  console.log('hello', remainingVol);
  let base = Math.sqrt(120*120 - 40*40);
  console.log('base of triangle', base);
  let remainingVol2 = ((.5*radius*radius*theta*2) - (base*heightOfLiq)) / (Math.PI*radius*radius);
  console.log(remainingVol2, 'after triangle removal');
  // vt * proportion of circle occupied by moon shape == > remaining vol
  ans = vt * (1 - remainingVol - remainingVol2);
  console.log(ans);
}
